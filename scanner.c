 ...
 * @author Huu-Duc Nguyen, Sy-Hung Vo  (K62 - SIE/SOICT)
 * @version 1.0.1
 */
...
Token *readConstChar(void)
{
	...
	else if (charCodes[currentChar] == EOF)
	{
		...
		state = 40;
		readChar();
		if((charCodes[currentChar] != CHAR_SINGLEQUOTE) && (charCodes[currentChar] != CHAR_BACKSLASH) && (charCodes[currentChar] != CHAR_DOUBLEQUOTE)) { // NOTE: only allow ' and \.
			token->tokenType = TK_NONE;
			error(ERR_INVALIDCHARCONSTANT, lineNo, colNo);
			return token;
		}
